...
function switchAction(newAction, element) {
    ...
    var posNewAction = actions.findIndex(x => x.name === newAction);
    if (posNewAction === -1) { 
        console.error('Animation clip not found');
        return;
    }

    var posCurrentAction = actions.findIndex(x => x.name === currentAction);

    if (posCurrentAction === -1) {
        actions[posNewAction].action.play();
    } else {
        if (newAction != currentAction && currentAction) {

            actions[posCurrentAction].action.fadeOut(0.3);
            actions[posNewAction].action.reset();
            actions[posNewAction].action.setEffectiveWeight( 1 );
            actions[posNewAction].action.play();
            actions[posNewAction].action.fadeIn(0.3);
...
document.addEventListener("keydown", (e) => {
    // throttle events
    if (currentKeyDown == e.which) return;
   
    currentKeyDown = e.which;
    const avatar = document.querySelector("#player");
    switch (currentKeyDown) {
      case 65: // left
        avatar.setAttribute("player-info", "clip:walk_forward;direction:left");
        // switchAction('walk_forward');
        break;

      case 87: // up
        avatar.setAttribute("player-info", "clip:walk_forward;direction:up");
        // switchAction('walk_forward');
        break;

      case 68: // right
        avatar.setAttribute("player-info", "clip:walk_forward;direction:right");
        // switchAction('walk_forward');
        break;

      case 83: // down
        avatar.setAttribute("player-info", "clip:walk_forward;direction:down");
        // switchAction('walk_forward');
        break;
    }
  });
  ...
  window.addEventListener("keyup", (e) => {
    ...
    switch (e.which) {
      case 65: // left
        avatar.setAttribute("player-info", "clip:idle");
        // switchAction('idle');
        break;
