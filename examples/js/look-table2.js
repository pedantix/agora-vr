	...
	AFRAME.registerComponent('look-at', {
	  ...
	  update: function () {
	    ...
	    if (typeof target === 'object') {
	      //console.log("target.x, target.y, target.z", target.x, target.y, target.z);
	  ...
	  lookAt: function (position) {
	    ...
	    if (this.el.getObject3D('camera')) {
	     ...
	     window.beno=object3D;
	     window.beno2=vector;
	    //console.log(" lookAt ", position, vector);
	    object3D.lookAt(vector);

	    var v3 = new THREE.Vector3();
	    var x=object3D.getWorldPosition(v3).x;
            if (x<-5) {
	    	object3D.rotateY(3.8);
	    } else if (x>5) {
	    	object3D.rotateY(2.4);
	    } else {
	    	object3D.rotateY(3.05);
	    }
	  }
	});
